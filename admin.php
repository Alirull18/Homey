      ...
      <div id="content">
        <div class="page">
          ...
          <div class="admin-tabs">
            <a href="admin.php?tab=analytics" class="admin-tab <?php echo $tab === 'analytics' ? 'active' : ''; ?>">Analytics Overview</a>
            <a href="admin.php?tab=approvals" class="admin-tab <?php echo $tab === 'approvals' ? 'active' : ''; ?>">Recipe Approvals (<?php echo $pending_count; ?>)</a>
            <a href="admin.php?tab=ingredients" class="admin-tab <?php echo $tab === 'ingredients' ? 'active' : ''; ?>">Ingredient Database</a>
            <a href="admin.php?tab=users" class="admin-tab <?php echo $tab === 'users' ? 'active' : ''; ?>">User Accounts</a>
          </div>
            ...
            <div class="g2">
              <div class="card">
                ...
                <div style="display:flex; flex-direction:column; gap:12px">
                  ...
                  $goals = ['Deficit' => 0, 'Surplus' => 0, 'Maintain' => 0];
                  $total_cohort = 0;
                  if ($res_goal) {
                      while ($row = mysqli_fetch_array($res_goal)) {
                          if (isset($goals[$row['goal_type']])) {
                              $goals[$row['goal_type']] = $row['count'];
                  ...
                  foreach ($goals as $name => $count):
                      ...
                      $cls = $name === 'Deficit' ? 'bar-g' : ($name === 'Surplus' ? 'bar-a' : 'bar-b');
                  ?>
                    <div>
                      <div style="display:flex; justify-content:space-between; font-size:12.5px; font-weight:700; margin-bottom:4px">
                        <span>Calorie <?php echo htmlspecialchars($name); ?></span>
                        <span style="color:var(--gray400)"><?php echo $count; ?> members (<?php echo $pct; ?>%)</span>
                      </div>
                      <div class="bar-wrap" style="height:10px"><div class="bar <?php echo $cls; ?>" style="width:<?php echo $pct; ?>%"></div></div>
                    </div>
                  <?php endforeach; ?>
                </div>
              </div>
              ...
              <div class="card">
                ...
                <table class="a-table">
                  <thead><tr><th>Database Item</th><th>Total Count</th></tr></thead>
                  <tbody>
                    <tr><td>Ingredient Records</td><td><?php
                      $res = mysqli_query($conn, "SELECT COUNT(*) as c FROM ingredients");
                      $row = mysqli_fetch_array($res); echo $row['c'];
                    ?> items</td></tr>
                    <tr><td>Hydration entries</td><td><?php
                      $res = mysqli_query($conn, "SELECT COUNT(*) as c FROM hydration_logs");
                      $row = mysqli_fetch_array($res); echo $row['c'];
                    ?> logs</td></tr>
                    <tr><td>Admin accounts</td><td><?php
          ...
          <?php elseif ($tab === 'approvals'): ?>
            ...
            <div class="card">
              <div class="card-title">Pending Community Recipe Approvals <span class="tag tag-r"><?php echo $pending_count; ?> pending</span></div>
              ...
              ?>
                <table class="a-table">
                  <thead><tr><th>Author</th><th>Recipe Title</th><th>Calories</th><th>Prep Time</th><th>Steps Summary</th><th>Actions</th></tr></thead>
                  <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          <details style="cursor:pointer; font-size:11.5px">
                            ...
                            <p style="margin:4px 0; white-space:pre-line; color:var(--gray600)"><?php echo htmlspecialchars($r['instructions']); ?></p>
                          </details>
                        </td>
                        <td>
              ...
              <?php else: ?>
                <p style="color:var(--gray400); font-size:13.5px; text-align:center; padding:30px 0; border:2px dashed var(--gray100); border-radius:8px">
                  All community submissions have been moderated. Clean plate!
                </p>
              <?php endif; 
              if ($res_pend) mysqli_free_result($res_pend);
              ?>
            </div>
              ...
              <div class="card">
                ...
                ?>
                  <div style="max-height: 350px; overflow-y: auto">
                    <table class="a-table">
                      <thead><tr><th>Ingredient</th><th>Cat</th><th>kcal</th><th>Macros</th><th>Action</th></tr></thead>
                      <tbody>
                ...
                <?php endif; 
                if ($res_ing) mysqli_free_result($res_ing);
                ?>
              </div>
            </div>
          ...
          <?php elseif ($tab === 'users'): ?>
            ...
            <div class="card">
              ...
              ?>
                <table class="a-table">
                  <thead><tr><th>Name</th><th>Email</th><th>Diet Goal</th><th>Plan Status</th><th>Actions</th></tr></thead>
                  <tbody>
                    <?php while ($u = mysqli_fetch_array($res_all_u)): ?>
                      <tr>
                        ...
                        <td><span class="tag tag-gray"><?php echo htmlspecialchars($u['goal_type'] ?? 'Deficit'); ?></span></td>
              ...
              <?php endif; 
              if ($res_all_u) mysqli_free_result($res_all_u);
              ?>
            </div>
          <?php endif; ?>
        </div>
