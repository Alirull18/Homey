        ...
        <div class="page">
          ...
          <div class="card" style="margin-bottom:16px">
            <form method="GET" action="recipes.php" style="display:flex;gap:8px;flex-wrap:wrap">
              <input name="search" class="form-input" placeholder="Search recipes by keyword..." style="flex:1; min-width:200px" value="<?php echo htmlspecialchars($search); ?>">
              
              <select name="meal_filter" class="form-input form-select" style="width:160px" onchange="this.form.submit()">
          ...
          <?php if (count($recipes) === 0): ?>
            <div style="background:var(--white); text-align:center; padding:50px 20px; border-radius:var(--radius); box-shadow:var(--shadow)">
              ...
              <?php foreach ($recipes as $r): ?>
                <div class="rcard" style="background:var(--white); border-radius:var(--radius); box-shadow:var(--shadow); display:flex; flex-direction:column; overflow:hidden">
                  <div class="rcard-body" style="padding:15px; display:flex; flex-direction:column; flex:1">
                    <div class="rcard-name" style="font-size:15px; font-weight:700; color:var(--gray800); margin-bottom:4px"><?php echo htmlspecialchars($r['title']); ?></div>
                    <div class="rcard-meta" style="font-size:12px; color:var(--gray400); margin-bottom:8px">
                      <?php echo $r['calories']; ?> kcal &middot; <?php echo $r['prep_time_min']; ?> min &middot; Shared by: <?php echo htmlspecialchars($r['author_name']); ?>
                    </div>
                    
                    <span class="tag tag-g" style="align-self:flex-start; margin-bottom:12px"><?php echo htmlspecialchars($r['meal_type']); ?></span>
                    
                    <!-- Form log makanan resipi secara langsung -->
                    <form method="POST" action="recipes.php" style="margin-bottom:12px; border-top:1px solid var(--gray100); padding-top:10px">
                      <input type="hidden" name="action" value="log_recipe_meal">
                      <input type="hidden" name="recipe_id" value="<?php echo $r['recipe_id']; ?>">
                      
                      <div style="display:flex; gap:6px; align-items:center">
                        <select name="log_meal_type" class="form-input form-select" style="font-size:11.5px; padding:4px 8px; height:32px; width:110px; line-height:1; margin:0">
                        ...
                        <button type="submit" class="btn btn-primary" style="flex:1; height:32px; font-size:11.5px; padding:0 8px; font-weight:700">Log Meal</button>
                      </div>
                    </form>
                    ...
                    <details style="margin-top:auto; padding-top:6px; border-top:1px dashed var(--gray100)">
                      <summary style="font-size:12.5px; font-weight:700; color:var(--g600); cursor:pointer">View Preparation Steps</summary>
                      <p style="font-size:12px; color:var(--gray600); line-height:1.6; margin-top:8px; white-space:pre-line">
