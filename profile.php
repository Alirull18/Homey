...
if ($is_new_user) {
    ...
    elseif ($bmi >= 25 && $bmi < 30) $bmi_status = 'Overweight';
    elseif ($bmi >= 30) $bmi_status = 'Obese';
}
    ...
    <div id="main">
      ...
      <div id="content">
        <div class="page">
          ...
          <div style="display:flex;gap:14px;align-items:center;background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow);padding:18px 20px;margin-bottom:16px">
            <div style="width:54px;height:54px;border-radius:50%;background:var(--g100);display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-size:18px;font-weight:800;color:var(--g700)"><?php echo htmlspecialchars($sidebar_initials); ?></div>
            <div>
              <div style="font-size:17px;font-weight:700;font-family:var(--font-display)"><?php echo htmlspecialchars($full_name); ?></div>
              <div style="font-size:13px;color:var(--gray400)"><?php echo htmlspecialchars($email); ?></div>
              <span class="tag tag-g" style="margin-top:4px;display:inline-flex"><?php echo htmlspecialchars($sidebar_role); ?> Plan</span>
            </div>
          </div>

          <div class="g3" style="margin-bottom:16px">
            <div class="card" style="text-align:center"><div class="metric-lbl">Days tracked</div><div class="metric-val"><?php echo $days_tracked; ?></div></div>
            <div class="card" style="text-align:center"><div class="metric-lbl">TDEE Target</div><div class="metric-val"><?php echo $targetKcal; ?> <small>kcal</small></div></div>
            <div class="card" style="text-align:center"><div class="metric-lbl">Recipes shared</div><div class="metric-val"><?php echo $recipes_shared; ?></div></div>
          </div>
                ...
                <form name="profileForm" method="POST" action="profile.php" onsubmit="return validateProfileForm()">
                  <div class="form-grid" style="margin-bottom:10px">
                    <div class="form-group">
                      ...
                      <input name="age" class="form-input" type="number" value="<?php echo $is_new_user ? '' : $age; ?>" placeholder="e.g. 24" required min="1" max="120">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <input name="weight" class="form-input" type="number" step="0.1" value="<?php echo $is_new_user ? '' : $weight_kg; ?>" placeholder="e.g. 75" required min="1">
                    </div>
                    <div class="form-group">
                      <label class="form-label">Height (cm)</label>
                      <input name="height" class="form-input" type="number" step="0.1" value="<?php echo $is_new_user ? '' : $height_cm; ?>" placeholder="e.g. 172" required min="1">
                    </div>
                  </div>
                  ...
                  <div class="form-group" style="margin-bottom:12px">
                    ...
                    <select name="goal_type" class="form-input form-select">
                      <option value="Deficit" <?php echo ($goal_type === 'Deficit') ? 'selected' : ''; ?>>Calorie Deficit (Weight Loss)</option>
                      <option value="Surplus" <?php echo ($goal_type === 'Surplus') ? 'selected' : ''; ?>>Calorie Surplus (Weight Gain)</option>
                      <option value="Maintain" <?php echo ($goal_type === 'Maintain') ? 'selected' : ''; ?>>Maintain Weight</option>
                    </select>
                  </div>

                  <div class="alert alert-b" style="margin-bottom:12px">
                    Calculated TDEE: <strong><?php echo $tdee; ?> kcal/day</strong> &middot; Recommended target: <strong><?php echo $targetKcal; ?> kcal/day</strong>
                  </div>

                  <button type="submit" class="btn btn-primary btn-full" style="font-weight:700">Save profile & recalculate goal</button>
                </form>
              </div>
